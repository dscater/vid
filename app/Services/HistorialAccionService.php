...
class HistorialAccionService
{
    ...
    public function modificarAccion(int $id, Model $modelo, Model $modelo_update = null, array $relaciones = []): void
    {
        if (!empty($relaciones)) {
            $datos_original = $modelo->loadMissing($relaciones)->makeHidden($modelo->getAppends())->toArray();
        } else {
    ...
    private function actualizaAccion(int $id, array $datos): void
    {
        $historial_accion = HistorialAccion::findOrFail($id);
        $historial_accion->update([
            // "user_id" => $datos["user_id"],
            // "accion" => $datos["accion"],
            // "descripcion" => $datos["descripcion"],
            'datos_original' => $datos["datos_original"],
